...
class BudgRubrique(models.Model):
	...
	no_imputation = fields.Many2one("compta_plan_lines", string = "Imputation Cptble", required=True)
	...
	def validation_budget(self):
		...
		for line in self.env.cr.dictfetchall():
			...
			if curs_verif_recette == 0:
				...
				self.env.cr.execute("""UPDATE budg_ligne_exe_rec SET mnt_corrige = mnt_corrige + %s,reste_emettre = reste_emettre + %s
		...
		for line in self.env.cr.dictfetchall():
			...
			if curs_verif_depense == 0:
				...
				self.env.cr.execute("""UPDATE budg_ligne_exe_dep SET mnt_corrige = mnt_corrige + %s, mnt_disponible = mnt_disponible + %s
				WHERE cd_titre_id = %s AND cd_section_id = %s AND  cd_chapitre_id = %s AND  cd_art_id = %s AND
				cd_paragraphe_id = %s AND cd_rubrique_id = %s AND x_exercice_id = %s AND company_id = %s """
				,(mnt_budgetise1,mnt_budgetise1,v_titre1,v_section1,v_chap1,v_article1,v_para1,v_rub1,val_struct,val_ex,))
...
class BudgEngagement(models.Model):
	...
	cd_rubrique_id = fields.Many2one("budg_rubrique", string = "Rubrique", required=True, states={'R': [('readonly', True)],'W': [('readonly', True)], 'L': [('readonly', True)],'O': [('readonly', True)]})
	...
	lb_obj = fields.Text(string = "Objet",size=300, required=True, states={'W': [('readonly', True)], 'V': [('readonly', True)], 'L': [('readonly', True)],'O': [('readonly', True)]})
	...
	mnt_lbudg_ap = fields.Integer(string = "Montant ligne budgetaire ap")
	...
	def action_eng_confirmer(self):
		...
		if self.mnt_eng > res:
			...
			WHERE cd_titre_id = %d AND cd_section_id = %d AND cd_chapitre_id = %d AND cd_art_id = %d AND cd_paragraphe_id = %d AND cd_rubrique_id = %d and company_id = %d and x_exercice_id = %d"""
			%(val_titre, val_sec, val_chap ,val_art, val_par,val_rub,val_struct,val_ex,val_titre, val_sec, val_chap, val_art, val_par,val_rub, val_struct, val_ex))
	...
	def mnt_nvo_dispo(self):
		...
		self.env.cr.execute("""select (mnt_corrige - mnt_engage) FROM budg_ligne_exe_dep l, budg_engagement e
		WHERE l.company_id = %d and l.x_exercice_id = %d and l.cd_titre_id = %d and l.cd_section_id = %d and l.cd_chapitre_id = %d
		and l.cd_art_id = %d and l.cd_paragraphe_id = %d and l.cd_rubrique_id = %d and e.id = %d
		""" %(val_struct,val_ex, val_titre, val_sec, val_chap, val_art, val_para, val_rub, v_id))
	...
	def cred_dipso(self):
		...
		result = self.env['budg_ligne_exe_dep'].search([('cd_paragraphe_id','=',val_para),('cd_rubrique_id','=',val_rubrique),('company_id','=',val_struct),('x_exercice_id','=',val_ex)])
...
class BudgLiqOrd(models.Model):
	...
	no_eng = fields.Many2one("budg_engagement", "Engagement", required=True,domain=[('state', '=', 'W')])
	...
	text_amount = fields.Char(string="Montant en lettre", required=False, compute="amount_to_words" )
	dt_etat = fields.Date(default=fields.Date.context_today, string="Date liquidation", readonly = True)
...
class BudgMandat(models.Model):
	...
	def afficher_piece(self):
		...
		for vals in self:
		    vals.env.cr.execute("""select b.lb_long as lib, b.oblige as obl, montant as mnt,  b.nombre as nbr, b.ref as ref from budg_piece_liq b where liq_id = %d""" %(val_liq))
		    rows = vals.env.cr.dictfetchall()
		    result = []
		    
		    vals.piecejust_ids.unlink()
		    for line in rows:
		        result.append((0,0, {'lb_long' : line['lib'], 'oblige': line['obl'], 'nombre': line['nbr'],'montant': line['mnt'],'ref': line['ref']}))
		    self.piecejust_ids = result
...
class BudgBordEng(models.Model):
	...
	x_exercice_id = fields.Many2one("ref_exercice", default=lambda self: self.env['ref_exercice'].search([('etat','=', 1)]), string="Exercice")
	company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id.id)
	engagement_ids = fields.Many2many("budg_engagement", "eng_id", domain=[('state','=','V')], string="Liste des engagements", ondelete="restrict")
	state = fields.Selection([
        ('N', 'Nouveau'),
		('T', 'Envoyé à DCMEF/CG'),
...
class BudgBordMan(models.Model):
	...
	x_exercice_id = fields.Many2one("ref_exercice", default=lambda self: self.env['ref_exercice'].search([('etat','=', 1)]), string="Exercice")
	company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id.id, string="Structure")
	mandat_ids = fields.Many2many("budg_mandat", "budg_detail_bord_mandat", domain=[('state','=','O')], string="Liste des mandats")
	et_doss = fields.Selection([
        ('N', 'Nouveau'),
        ('V', 'Envoyé à DCMEF/CG'),
        ('R', 'Réceptionné par DCMEF/CG')
...
class Ordre_Paiement(models.Model):
	...
	no_cpt_tiers = fields.Many2one("compta_plan_lines",string="Compte de tiers", required=False)
...
class Budg_Balance(models.Model):
	...
	def remplir_balance(self):
		...
		for vals in self:
			vals.env.cr.execute("""SELECT DISTINCT br.rubrique as code, br.lb_long as libelle, e.mnt_corrige as montant, en.mnt_eng as engagement, bm.mnt_ord as mandat 
			...
			AND en.state <> 'A' AND en.x_exercice_id = %s and en.company_id = %s and en.dt_etat BETWEEN %s and %s order by code asc""" ,(val_ex, val_struct, val_deb, val_fin))
			rows = vals.env.cr.dictfetchall()
			result = []
			
			vals.balance_lines.unlink()
			for line in rows:
				result.append((0,0, {'numero_compte' : line['code'], 'libelle': line['libelle'], 'montant_budgetise': line['montant'], 'montant_engagement': line['engagement'], 'montant_mandatement': line['mandat']}))
...
class Budg_BalanceRecette(models.Model):
	...
	def remplir_balance(self):
		...
		for vals in self:
			vals.env.cr.execute("""SELECT DISTINCT p.paragraphe as code, rp.lb_long as libelle, r.mnt_budgetise as montant, re.mnt_rec as emis
			FROM budg_ligne_exe_rec r, budg_paragraphe p, budg_titrerecette re, ref_paragraphe rp 
			WHERE r.cd_paragraphe_id = p.id AND re.cd_paragraphe_id = p.id AND rp.id = p.paragraphe AND r.x_exercice_id = %s and r.company_id = %s and re.dt_rec BETWEEN %s and %s order by code """ ,(val_ex, val_struct, val_deb, val_fin))
			rows = vals.env.cr.dictfetchall()
			result = []
...
class Budg_synthese(models.Model):
	...
	etat = fields.Selection([
		('draft', 'Engagement brouillon'),
		('N', 'Engagement nouveau'),
		('L', 'Engagement liquidé'),
		('A', 'Engagement annulé'),
		('V', 'Engagement approuvé par DAF/DFC'),
		('W', 'Engagement visé par DCMECF/CG'),
		('R', 'Engagement rejeté par DCMECFCF/CG')
		], 'Etat')
	rubrique_lines = fields.One2many("budg_synthese_rubrique_line", 'synthese_id')
	x_exercice_id = fields.Many2one("ref_exercice", default=lambda self: self.env['ref_exercice'].search([('etat','=', 1)]), string="Exercice")
	company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id.id)
	...
	def remplir_synthese(self):
		...
		if self.cd_rubrique_id:
			...
			for vals in self:
				vals.env.cr.execute("""SELECT DISTINCT e.no_eng as numero, e.dt_etat as dte, e.lb_obj as objet, e.mnt_eng as montant, cd_rubrique_id as rub
				FROM budg_engagement e WHERE e.cd_rubrique_id = %s AND 
				e.x_exercice_id = %s AND e.company_id = %s AND e.dt_etat BETWEEN %s and %s order by e.no_eng """ ,(rub,val_ex, val_struct, val_deb, val_fin))
				rows = vals.env.cr.dictfetchall()
				result = []

				vals.rubrique_lines.unlink()
				for line in rows:
					result.append((0,0, {'noeng' : line['numero'], 'dte': line['dte'],'objet': line['objet'], 'montant': line['montant'], 'rubrique': line['rub']}))
			...
			for vals in self:
				vals.env.cr.execute("""SELECT DISTINCT e.no_eng as numero, e.dt_etat as dte, e.lb_obj as objet, e.mnt_eng as montant, cd_rubrique_id as rub
				FROM budg_engagement e WHERE e.x_exercice_id = %d AND e.company_id = %d order by e.no_eng """ %(val_ex, val_struct))
				rows = vals.env.cr.dictfetchall()
				result = []
...
class Budg_synthese_liq(models.Model):
	...
	tout = fields.Boolean("Toutes les liquidations")
	etat = fields.Selection([
		('draft', 'Liquidation brouillon'),
		('N', 'Liquidation état nouveau'),
		('L', 'Liquidation certifiée'),
		('A', 'Liquidation annulé'),
		], 'Etat', required=False)
	...
	def remplir_synthese_liq(self):
		...
		if self.tout == True:
			for vals in self:
				vals.env.cr.execute("""SELECT DISTINCT e.id as numero, l.no_lo as liq, e.lb_obj as objet, e.mnt_eng as montant, e.state as etat
				FROM budg_engagement e, budg_liqord l
				WHERE  e.x_exercice_id = %s AND e.company_id = %s and e.id = l.no_eng """ ,(val_ex, val_struct))
				rows = vals.env.cr.dictfetchall()
				result = []

				vals.rubrique_liq_lines.unlink()
				for line in rows:
					result.append((0,0, {'noeng' : line['numero'], 'noliq': line['liq'], 'objet': line['objet'], 'montant': line['montant'], 'etat': line['etat']}))
			...
			for vals in self:
				vals.env.cr.execute("""SELECT DISTINCT e.id as numero, l.no_lo as liq, e.lb_obj as objet, e.mnt_eng as montant, e.state as etat
				FROM budg_engagement e, budg_liqord l
				WHERE e.x_exercice_id = %s AND e.company_id = %s AND
				e.id = l.no_eng AND e.dt_etat BETWEEN %s AND %s """, ( val_ex, val_struct, val_deb,val_fin))
				rows = vals.env.cr.dictfetchall()
				result = []

				vals.rubrique_liq_lines.unlink()
				for line in rows:
					result.append((0, 0, {'noeng': line['numero'], 'noliq': line['liq'], 'objet': line['objet'],
										  'montant': line['montant'], 'etat': line['etat']}))
				self.rubrique_liq_lines = result
			...
			for vals in self:
				vals.env.cr.execute("""SELECT DISTINCT e.id as numero, l.no_lo as liq, e.lb_obj as objet, e.mnt_eng as montant, e.state as etat
				FROM budg_engagement e, budg_liqord l
				WHERE  e.x_exercice_id = %s AND e.company_id = %s and e.id = l.no_eng and l.state = %s """, (val_ex, val_struct, val_etat))
				rows = vals.env.cr.dictfetchall()
				result = []
...
class Budg_synthese_liq_line(models.Model):
	...
	noeng = fields.Many2one("budg_engagement","N° eng")
	noliq = fields.Many2one("budg_liqord","N° liq")
...
class Budg_synthese_mdt(models.Model):
	...
	mandat_liq_lines = fields.One2many("budg_synthese_mdt_rubrique_line", 'synthese_mdt_id')
	x_exercice_id = fields.Many2one("ref_exercice", default=lambda self: self.env['ref_exercice'].search([('etat','=', 1)]), string="Exercice")
	company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id.id)
	...
	def remplir_mdt_liq(self):
		...
		val_deb = str(self.dt_appro_deb)
		val_fin = str(self.dt_appro_fin)
		val_titre = str(self.cd_titre_id.titre.titre)
		val_section = str(self.cd_section_id.section.section)
		val_chap = str(self.cd_chapitre_id.chapitre.chapitre)
		val_art = str(self.cd_article_id.article.article)
		val_para = str(self.cd_paragraphe_id.paragraphe.paragraphe)
		val_rub = str(self.cd_rubrique_id.concate_rubrique)
		val_etat = str(self.etat)
		
		for vals in self:
			vals.env.cr.execute("""SELECT DISTINCT m.no_mandat as numero, m.obj as objet, m.mnt_ord as montant, m.state as etat
			FROM budg_engagement e, budg_mandat m
			...
			AND m.x_exercice_id = %s AND m.company_id = %s AND e.state = %s
			AND e.dt_etat BETWEEN %s AND %s""" ,(val_titre, val_section, val_chap, val_art, val_para, val_rub, val_ex, val_struct, val_etat,val_deb, val_fin))
			rows = vals.env.cr.dictfetchall()
			result = []
...
class Budg_journal_eng(models.Model):
	...
	def remplir_jnl_eng(self):
		...
		for vals in self:
			...
			AND e.dt_etat BETWEEN %s and %s AND e.no_eng = m.no_eng AND b.id = e.no_beneficiaire""" ,(val_ex, val_struct,val_deb, val_fin))
			rows = vals.env.cr.dictfetchall()
			result = []
...
class Budg_journal_mdt(models.Model):
	...
	def remplir_jnl_mdt(self):
		...
		for vals in self:
			...
			FROM budg_engagement e, budg_paragraphe p, budg_mandat m, ref_beneficiaire b,ref_paragraphe rp
			WHERE rp.id = p.paragraphe and p.id = e.cd_paragraphe_id AND e.x_exercice_id = %s AND e.company_id = %s AND b.id = e.no_beneficiaire
			AND e.dt_etat BETWEEN %s and %s AND e.no_eng = m.no_eng and m.state != 'A' and m.state != 'R' """ ,(val_ex, val_struct,val_deb, val_fin))
			rows = vals.env.cr.dictfetchall()
			result = []
...
class BudgAnnulMandat(models.Model):
	...
	def annuler(self):
		...
		v_eng = int(self.mandat_id.no_eng)
		...
		self.env.cr.execute("""UPDATE budg_engagement SET state = 'V' WHERE x_exercice_id = %s AND company_id = %s AND no_eng = '0009'""" , (v_ex, v_struct))
...
class BudgListeMdt(models.Model):
	...
	def viser(self):
		...
		self.env.cr.execute("""UPDATE budg_mandat SET state ='V' WHERE id = %d and company_id = %d and
	...
	def afficher_piece(self):
		...
		for vals in self:
			vals.env.cr.execute("select lb_long as lib, oblige as obl, ref as re, montant as mnt, nombre as nbr from budg_piece_liq where liq_id = %d and company_id = %d and x_exercice_id = %d" %(liq, val_struct, val_ex))
...
class BudgBordMdtControle(models.Model):
	...
	et_doss = fields.Selection([
		...
		('PC', 'Receptionné pour prise en charge'),
        ], 'Etat', default='N', required=True)
	total_prec = fields.Integer()
	...
	def receptionner(self):
		for vals in self:
			...
			val_struct = int(vals.company_id.id)
			"""
			self.env.cr.execute("UPDATE budg_bordereau_mandatement SET state ='R' WHERE id = %d and x_exercice_id = %d and
			company_id = %d" %(bord, val_ex, val_strcut))
			"""

			vals.env.cr.execute("select bordmand from budg_compteur_bord_mand where x_exercice_id = %d and company_id = %d" %(val_ex, val_struct))
			bordmand = self.env.cr.fetchone()
			no_bord_mandat = bordmand and bordmand[0] or 0
			c1 = int(no_bord_mandat) + 1
			c = str(no_bord_mandat)
	...
	def afficher(self):
		...
		for vals in self:
			...
			where m.company_id = %d and m.x_exercice_id = %d and m.state = 'V' """ %(val_struct, val_ex))
...
class BudgAnnulationEngagement(models.Model):
	...
	def AnnulerEngagement(self):
		...
		for val in self:
			...
			self.env.cr.execute(
				"""UPDATE budg_engagement SET state = 'A' WHERE id = %d and x_exercice_id = %d and company_id = %d""" % (
				eng, no_ex, struct))
		...
		for vals in self:
			...
			for line in rows:
				result.append((0, 0, {'titre': line['cd_titre_id'], 'section': line['cd_section_id'],
									  ...
									  'engagement': line['mnt_engage'], 'mandat': line['mnt_mandate'], 'taux': line['taux'], 'reste': line['reste_mandat']}))
