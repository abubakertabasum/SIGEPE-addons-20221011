...
class Facturation(models.Model):
    ...
    objet = fields.Text("POUR", required=True)
    accompagne = fields.Integer("Accompagnement RTB", required=False)
    ...
    etat = fields.Selection([('N','Nouveau'),('P','Proforma'),('V','Définitve')],string="Etat")
    ...
    def calculer(self):
        ...
        self.env.cr.execute("""SELECT sum(l.prix_total)
        FROM compta_facturation_ligne l, compta_facturation f WHERE x_exercice_id = %d AND company_id = %d AND facture_id = %d """ %(val_ex, val_struct, id_ordre))
        res = self.env.cr.fetchone()
        val_mnt = res and res[0] or 0 
        self.ht = val_mnt - self.accompagne
        self.tva = round(self.ht)
...
class FacturationLigne(models.Model):
    ...
    facture_id = fields.Many2one("compta_facturation", ondelete='cascade')
    unite = fields.Many2one("compta_unite", "Unité")
    forfait = fields.Boolean("Forfait", default=False)
    qte = fields.Integer("Quantité", default=1)
    prix_unitaire = fields.Integer("Prix Unitaire", readonly=False)
    prix_total = fields.Integer("Total",readonly=True)
    categorie_id = fields.Many2one("compta_categorie", "Catégorie de prestation", required=True)
    type_id = fields.Many2one("compta_type_prestation", "Type de prestation", required=True)
    designation = fields.Many2one("compta_prestation", "Prestation", required=True)
    rtb = fields.Selection([("1","RTB1"),("2","RTB2")], string="Choisir le")
    ...
    @api.onchange("parametre_id")
    def Onchangeparam(self):
        
        for x in self:
            if x.forfait == False:
                x.prix_unitaire = x.parametre_id.prix_unitaire
                x.prix_total = x.prix_unitaire * x.qte
            else:
                x.prix_unitaire = x.prix_unitaire
                x.prix_total = x.prix_unitaire
