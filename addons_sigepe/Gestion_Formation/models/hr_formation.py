...
class HrPlanFormationLine(models.Model):
    ...
    _rec_name = 'x_theme'
    x_plan_id = fields.Many2one('hr_plan_formation')
    ...
    mnt_a_soustrair = fields.Integer(compute = 'remplir_mnt', string = 'Montant', store = True)
...
class HrSaisieInformationStage(models.Model):
    ...
    x_fonction_id = fields.Many2one('hr_fonction', string = 'Fonction', states={'CL': [('readonly', True)]})
...
class HrParticipants(models.Model):
    ...
    active = fields.Boolean(string = "Etat", default=True)
    x_line_ids = fields.One2many('hr_participant_line','name', string = "Liste des Participants")
...
class HrParticipantsRetenus(models.Model):
    ...
    x_service = fields.Char(string = 'Service') 
    active = fields.Boolean(string = "Etat", default=True)
    x_line_ids = fields.One2many('hr_participant_retenus_line','name', string = "Liste des Participants Retenus")
...
class HrPlanFormationTrienal(models.Model):
    ...
    x_line_p_ids = fields.One2many('hr_plan_besoinservice_trienal_line','x_service_besoin_id', string = 'Liste Des Services demandeurs', readonly = False)
    state = fields.Selection([
        ('draft', 'Brouillon'),
        ('R', 'Rechercher'),
        ('V', 'Valider'),
...
class HrPlanFormationTrienalLine(models.Model):
    ...
    _rec_name = 'x_theme'
    x_plan_id = fields.Many2one('hr_plan_formation_trienal')
...
class HrPlanBesoinsAnnuelsTrienalLineService(models.Model):
    ...
    x_service_besoin_id = fields.Many2one('hr_plan_formation_trienal')
    domaine = fields.Many2one('hr_domaine', string = 'Domaine')
    x_theme = fields.Many2one('hr_theme', string = 'Thème')
    service_demandeur = fields.Many2one('hr_service', string = 'Service demandeur')
    company_id = fields.Many2one('res.company',string = "Structure", default=lambda self: self.env.user.company_id.id)
