...
class HrEmployeePieceDetachement(models.Model):
    ...
    def compute_annee_detachement(self):
        if self.date_effet:
            ...
            date2 = self.date_effet.year + 5
            if self.date_effet.day == 1:
                if self.date_effet.month == 1:
                    jour = 31
                    annee = self.date_effet.year + 5 - 1
...
class Contract(models.Model):
    ...
    x_total_indemnites = fields.Float(string = "Total Indemn", related = "employee_id.x_total_indemnites")
    ...
    x_net_payer_ctrct = fields.Float(string="Net à payer", readonly=False, store=True)
...
class HrEmployee(models.Model):
    ...
    @api.depends('charge_enfant','x_lib_type_emp')
    def mnt_alloc(self):
        for vals in self:
            typ_emp = vals.x_lib_type_emp  
            print("Type employé", typ_emp)
            if typ_emp == 'Contractuel' or typ_emp == 'Hospitalo-Universitaire':
    ...
    @api.depends('x_remuneration_total','mnt_total_rappel')
    ...
    @api.depends('mnt_rappel_salaire','mnt_rappel_resp','mnt_rappel_astr','mnt_rappel_loge','mnt_rappel_techn','mnt_rappel_spec','mnt_rappel_it','mnt_rappel_ifc','mnt_rappel_irp','mnt_rappel_trans','mnt_rappel_inf','mnt_rappel_explot','mnt_rappel_resp_financ','mnt_rappel_allocation','mnt_rappel_garde','mnt_rappel_risque','mnt_rappel_sujetion','mnt_rappel_formation','mnt_rappel_caisse','mnt_rappel_veste')
    ...
    mnt_total_retenues = fields.Float(compute = 'total_retenue_field', string = 'Total retenues')
    
    #fonction qui permet d'avoir le montant Total des retenues des employés 
    @api.depends('x_mnt_carfo','x_iuts_net','x_mnt_cnss','mnt_total_trop_percu','x_mnt_taux_retenu_emolmt','x_retrait_prcpt_mois')
        ...
        for val in self:
            if val.x_type_employe_id.name == 'Fonctionnaire Detaché' or val.x_type_employe_id.name == 'fonctionnaire detaché' or val.x_type_employe_id.name == 'FONCTIONNAIRE DETACHE' or val.x_type_employe_id.name == 'Fonctionnaire Mis à Disposition' or val.x_type_employe_id.name == 'FONCTIONNAIRE MIS A DISPOSITION':
                if self.x_precompte != 0:
                    val.mnt_total_retenues = round(val.x_mnt_carfo + val.x_iuts_net + val.mnt_total_trop_percu + val.x_retrait_prcpt_mois)
                else:
                ...
                if self.x_precompte != 0:
                    val.mnt_total_retenues = round(val.x_mnt_cnss + val.x_iuts_net + val.mnt_total_trop_percu + val.x_retrait_prcpt_mois)
                else:
                    val.mnt_total_retenues = round(val.x_mnt_cnss + val.x_iuts_net + val.mnt_total_trop_percu)
            else:
                if self.x_precompte != 0:
                    val.mnt_total_retenues = round(val.x_mnt_taux_retenu_emolmt + val.mnt_total_trop_percu + val.x_retrait_prcpt_mois + val.x_iuts_net)
                else:
                    val.mnt_total_retenues = round(val.x_mnt_taux_retenu_emolmt + val.mnt_total_trop_percu + val.x_iuts_net)
    ...
    #Total indemnités = Total de toutes les indemnités auxquelles l'employé à droit
    x_total_indemnites = fields.Float(compute='depend_field',string = "Total Indemn", readonly = True)
    
    x = fields.Float(compute='',string = "Total Indemnité", readonly = True, store=True)
    
    #Remuneration total = Salaire de base + Total indemnités
    x_remuneration_total = fields.Float(compute = 'renum_total_field', string = "Rémuneration totale", readonly = True, store = True)
    ...
    x_prime_exo = fields.Float(compute = 'calcul_exoneration_prime', store = True,string = "Exo.Prime Rendement", readonly = False)
    ...
    @api.depends('x_solde_indiciaire_ctrct')
    ...
    @api.depends('x_remuneration_total','x_mnt_carfo','x_mnt_cnss')
    def calcul_salaire_brut(self):
        for vals in self:
            type_emp = vals.x_lib_type_emp
            if type_emp == "Contractuel" or type_emp == "CONTRACTUEL":
                vals.x_salaire_brut = vals.x_remuneration_total - vals.x_mnt_cnss
            else:
                vals.x_salaire_brut = vals.x_remuneration_total - vals.x_mnt_carfo
    ...
    def calcul_exoneration_logement(self):
        for vals in self:
            ...
            vals.x_indem_loge_exo = min([premiere_limite,vals.x_indem_loge,75000])
    ...
    def calcul_exoneration_transport(self):
        for vals in self:
            ...
            vals.x_indem_transp_exo = min([premiere_limite,vals.x_indem_transp,30000])
    ...
    def calcul_exoneration_responsabilite(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_resp_exo = min([premiere_limite,vals.x_indem_resp,50000])
    ...
    def calcul_exoneration_astreintes(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_astr_exo = min([premiere_limite,vals.x_indem_astr,50000])
    ...
    def calcul_exoneration_technicites(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_techn_exo = min([premiere_limite,vals.x_indem_techn,50000])
    ...
    def calcul_exoneration_specifiques(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_specif_exo = min([premiere_limite,vals.x_indem_specif,50000])
    ...
    def calcul_exoneration_specifiques_it(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_specif_it_exo = min([premiere_limite,vals.x_indem_spec_inspect_trav,50000])
 ...
 #indemnités de spécifiques IRP
    ...
    def calcul_exoneration_specifiques_irp(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_specif_irp_exo = min([premiere_limite,vals.x_indem_spec_inspect_irp,50000])
    ...
    def calcul_exoneration_specifiques_ifc(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_specif_ifc_exo = min([premiere_limite,vals.x_indem_spec_inspect_ifc,50000])
    ...
    def calcul_exoneration_informatique(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_inform_exo = min([premiere_limite,vals.x_indem_inform,50000])
    ...
    def calcul_exoneration_exploitation(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_exploit_exo = min([premiere_limite,vals.x_indem_exploit,50000])
    ...
    def calcul_exoneration_resp_finance(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_finance_exo = min([premiere_limite,vals.x_indem_finance,50000])
    ...
    def calcul_exoneration_garde(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_garde_exo = min([premiere_limite,vals.x_indem_garde,50000])
    ...
    def calcul_exoneration_risque(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_risque_exo = min([premiere_limite,vals.x_indem_risque,50000])
    ...
    def calcul_exoneration_sujetion(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_suj_exo = min([premiere_limite,vals.x_indem_suj,50000])
    ...
    def calcul_exoneration_formation(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_form_exo = min([premiere_limite,vals.x_indem_form,50000])
    ...
    def calcul_exoneration_caisse(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_caisse_exo = min([premiere_limite,vals.x_indem_caisse,50000])
    ...
    def calcul_exoneration_veste(self):
        for vals in self:
            premiere_limite = round((vals.x_salaire_brut * 5) / 100)
            vals.x_indem_veste_exo = min([premiere_limite,vals.x_indem_veste,50000])
    ...
    @api.depends('x_mnt_carfo','x_iuts_net','x_mnt_cnss','x_retrait_prcpt_mois')
    ...
    """@api.depends('x_solde_indiciaire_ctrct', 'x_total_indemnites','x_emolument_ctrct_net','x_allocation_familial')
    ...
    @api.onchange('x_solde_indiciaire_ctrct', 'x_total_indemnites','x_emolument_ctrct_net','x_allocation_familial')
    def renum_total_field(self):
        for val in self: 
            #val.x_remuneration_total = round(val.x_solde_indiciaire_ctrct + val.x_total_indemnites + val.x_emolument_ctrct_net + val.x_allocation_familial)
            val.x_remuneration_total = round(val.x_solde_indiciaire_ctrct + val.x_total_indemnites + val.x_emolument_ctrct_net + val.x_allocation_familial)

            #val.x_remuneration_total =5000"""
    ...
    @api.depends('x_remuneration_total')
    ...
    @api.depends('x_remuneration_total','x_mnt_carfo','x_iuts_net','mnt_total_rappel','mnt_total_trop_percu')
    def net_payer_field(self):
        for val in self:
                val.x_net_payer = round(val.x_remuneration_total - val.x_iuts_net - val.x_mnt_carfo + val.mnt_total_rappel - val.mnt_total_trop_percu) 
                 
    
    #fonction qui permet d'avoir le montant net à payer pour les contractuels
    @api.onchange('x_remuneration_total','x_mnt_cnss','x_mnt_carfo','x_iuts_net','mnt_total_rappel','mnt_total_trop_percu','x_retrait_prcpt_mois')
    def net_payer_ctrct_field(self):
        for val in self:
            if val.x_type_employe_id.name == 'Fonctionnaire Detaché' or val.x_type_employe_id.name == 'fonctionnaire detaché' or val.x_type_employe_id.name == 'FONCTIONNAIRE DETACHE' or val.x_type_employe_id.name == 'Fonctionnaire Mis à Disposition' or val.x_type_employe_id.name == 'FONCTIONNAIRE MIS A DISPOSITION':
                if self.x_precompte != 0:
                    val.x_net_payer_ctrct = round(val.x_remuneration_total - val.x_iuts_net - val.x_mnt_carfo + val.mnt_total_rappel - val.mnt_total_trop_percu - val.x_retrait_prcpt_mois)
                else:
                    val.x_net_payer_ctrct = round(val.x_remuneration_total - val.x_iuts_net - val.x_mnt_carfo + val.mnt_total_rappel - val.mnt_total_trop_percu)
            elif val.x_type_employe_id.name == 'Contractuel' or val.x_type_employe_id.name == 'CONTRACTUEL' or val.x_type_employe_id.name == 'contractuel':
                if self.x_precompte != 0:
                    val.x_net_payer_ctrct = round(val.x_remuneration_total - val.x_iuts_net - val.x_mnt_cnss + val.mnt_total_rappel - val.mnt_total_trop_percu - val.x_retrait_prcpt_mois)
                else:
                    val.x_net_payer_ctrct = round(val.x_remuneration_total - val.x_iuts_net - val.x_mnt_cnss + val.mnt_total_rappel - val.mnt_total_trop_percu)
            else:
                if self.x_precompte != 0:
                    val.x_net_payer_ctrct = round(val.x_remuneration_total + val.mnt_total_rappel - val.mnt_total_trop_percu - val.x_retrait_prcpt_mois)
                else:
                    val.x_net_payer_ctrct = round(val.x_remuneration_total + val.mnt_total_rappel - val.mnt_total_trop_percu)
...
#fonction de recherche permettant de retourner l'indemnité de caisse en fonction des paramètres
    ...
    def indem_caisse(self):
            ...
            if val_fonct != False:
                if val_zone != False or val_zone == False and val_echel != False or val_echel == False and val_cat != False or val_cat == False and val_struct != False:
                    res = self.env['hr_paramindemnitecaisse'].search([('x_fonction_id','=',val_fonct),('x_categorie_c_id','=',val_cat),('company_id','=',val_struct)])
