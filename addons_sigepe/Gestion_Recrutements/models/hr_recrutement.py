...
class HrSimulationSalaire(models.Model):
     ...
     x_fonction_id = fields.Many2one("hr_fonction",string ="Fonction", required = True, default=lambda self: self.env['hr_fonction'].search([('name','=', 'Choisir Fonction')]))
    ...
    #Remuneration total = Salaire de base + Total indemnités
     x_remu_total = fields.Float(compute = 'renum_total_field', string = "Rémuneration totale", readonly = False, store = True)
   
    #Total indemnités = Total de toutes les indemnités auxquelles l'employé à droit
     x_total_indemnites = fields.Float(compute = 'depend_field', string = "Total Indemn", readonly = False, store = True)
   
    
    
    
     #fonction qui permet d'additionner les indemnités
     @api.depends('x_indem_resp', 'x_indem_astr','x_indem_techn','x_indem_specif','x_indem_loge','x_indem_transp','x_indem_inform','x_indem_exploit','x_indem_finance','x_indem_garde','x_indem_risque','x_indem_suj','x_indem_form','x_indem_caisse','x_indem_veste')
     ...
     @api.depends('x_solde_indiciaire_ctrct', 'x_total_indemnites')
