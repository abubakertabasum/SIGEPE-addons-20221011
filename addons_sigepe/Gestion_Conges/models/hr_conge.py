...
class HrDemandeConge(models.Model):
    ...
    x_type_conge_id = fields.Many2one('hr_typeconge', string = 'Type de congé', required = True,states={'A': [('readonly', True)],'C': [('readonly', True)],'Re': [('readonly', True)],'An': [('readonly', True)]})
    ...
    x_nbre_jours = fields.Integer(compute = '_nombre_jours_dure', string = 'Durée')
    ...
    x_fonction = fields.Many2one('hr_fonction', string = 'Fonction',states={'Re': [('readonly', True)],'An': [('readonly', True)]})
    ...
    current_user = fields.Many2one('res.users','Utilisateur', default=lambda self: self.env.user,states={'Re': [('readonly', True)],'An': [('readonly', True)]})
    active = fields.Boolean(string = "Etat", default=True)
    ...
    @api.depends('x_nbre_jours1','nb_sous')
    ...
    def _nbr_restant_conge(self):
        if self.x_type_conge_id.name == "Congé Administratif":
            for vals in self:
                x_type_id = int(vals.x_type_conge_id)
                x_user_id = int(vals.current_user)
                vals.env.cr.execute("""select (E.x_compte_conge) as nbre from hr_employee E, hr_compte_conge C, hr_compte_conge_line CL, hr_typeconge T where C.id = CL.name and E.id = CL.x_employe_id and T.id = %d and E.user_id = %d"""%(x_type_id,x_user_id))
            ...
            for vals in self:
                x_type_id = int(vals.x_type_conge_id)
                x_user_id = int(vals.current_user)
                vals.env.cr.execute("""select (E.x_compte_conge_maternite) as nbre from hr_employee E, hr_compte_conge C, hr_compte_conge_line CL, hr_typeconge T where C.id = CL.name and E.id = CL.x_employe_id  and T.id = %d and E.user_id = %d"""%(x_type_id,x_user_id))
                res = vals.env.cr.fetchone()
                self.reste = res and res[0] or 0
            ...
            for vals in self:
                x_type_id = int(vals.x_type_conge_id)
                x_user_id = int(vals.current_user)
                vals.env.cr.execute("""select (E.x_compte_examens) as nbre from hr_employee E, hr_compte_conge C, hr_compte_conge_line CL, hr_typeconge T where C.id = CL.name and E.id = CL.x_employe_id  and T.id = %d and E.user_id = %d"""%(x_type_id,x_user_id))
                res = vals.env.cr.fetchone()
                self.reste = res and res[0] or 0
    ...
    def action_valider(self):
        if self.x_nbre_jours1 == 0:
            ...
            raise ValidationError(_('Votre compteur est à 0...imposiible pour vous de demander quoi que ce soit, adressez vous à qui de droit svp!!!!'))
        else:
    ...
    def action_appr(self):
        ...
        if self.x_type_conge_id.name == "Congé Administratif":
            for record in self:
                record.env.cr.execute("UPDATE hr_employee SET x_compte_conge = x_compte_conge - %d  WHERE user_id = %d" %(x_nbre,x_user_id))
            ...
            if self.reste != 0:
                for record in self:
                    record.env.cr.execute("UPDATE hr_employee SET x_compte_conge_maternite = x_compte_conge_maternite - %d  WHERE user_id = %d" %(x_nbre,x_user_id))
                    if record.report == 'Oui':
                        record.x_date_debut_att = record.x_date_debut_report
                        record.x_date_fin_att = record.x_date_fin_report
                    else:
            ...
            if self.reste != 0:
                for record in self:
                    record.env.cr.execute("UPDATE hr_employee SET x_compte_examens = x_compte_examens - %d  WHERE user_id = %d" %(x_nbre,x_user_id))
                    if record.report == 'Oui':
                        record.x_date_debut_att = record.x_date_debut_report
                        record.x_date_fin_att = record.x_date_fin_report
                    else:
...
class HrDemandeAutoAbsence(models.Model):
    ...
    x_nbre_jours = fields.Integer(compute = '_nombre_jours_dure', string = 'Duréé', readonly = True)
...
class HrCompteConge(models.Model):
    ...
    x_type_conge_id = fields.Many2one('hr_typeconge', string = 'Type de congé', required = True)
    x_nbre_jr_alloue = fields.Integer(string = 'Jour Alloué', required = True,default=30)
    x_nbre_jr_auto_abs = fields.Integer(string = 'Jour Absence', required = True,default=30)
    x_nbre_jr_maternite = fields.Integer(string = 'Jour Maternité', required = True,default=30)
    x_nbre_jr_examen = fields.Integer(string = 'Jour Examen', required = True,default=30)
    ...
    def action_valider(self):
        if self.x_exercice_id:
            ...
            for vals in self:
                vals.env.cr.execute("""select (E.id) as id, (E.name) as employe,(D.name) as direction, (S.name) as service from hr_employee E, hr_department D, hr_service S where E.x_direction_id = D.id and E.hr_service = S.id and S.id = %d and E.company_id = %d""" %(x_service_id,x_struct_id))
                rows = vals.env.cr.dictfetchall()
                result = []
                ...
                for line in rows:
                    result.append((0, 0, {'x_employe_id': line['id'],'x_employe': line['employe'],'x_direction': line['direction'],'x_service': line['service'],'x_nbre_jr': x_nbre_administratif,'x_nbre_jr_auto_abs': x_nbre_jr_auto_abs,'x_nbre_jr_maternite': x_nbre_jr_maternite,'x_nbre_jr_examen': x_nbre_jr_examen}))
    ...
    def action_appr(self):
        ...
        for record in self.x_line_ids:
            ...
            self.env.cr.execute("UPDATE hr_employee SET x_compte_conge =  %d,x_compte_conge_maternite = %d,x_compte_auto_abs = %d,x_compte_examens = %d WHERE id = %d" %(val_nbre,val_nbre_maternite,val_nbre_auto,val_nbre_examen,val_id))
        self.write({'state': 'A'})
